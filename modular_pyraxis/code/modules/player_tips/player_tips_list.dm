...
/datum/player_tips/proc/pick_tip(var/isSpecific)
	...
	switch(choice)
		if("general")
			var/info = "The following is a general tip for playing on EIS Pyraxis! You can disable them using the periodic tips toggle in the Global tab of the character setup! \n"
			return pick(
			prob(60); "[info] Got a question about gameplay, roleplay or the setting? Press F1 to Mentorhelp!",
			prob(60); "[info] We're based off Chompstation, and they have a wiki that is actively updated! Please check it out at https://wiki.chompstation13.net/index.php/Chomp_Station_Wiki for help!",
		...
		if("gameplay")
			var/info = "The following is a gameplay-focused tip for playing on EIS Pyraxis! You can disable them using the periodic tips toggle in the Global tab of the character setup! \n"
			return pick(
				...
				prob(50); "[info] Lost on the map? You can find In-Character help by speaking on the Common Radio. You can do this by pressing F3 and typing ' ; ' before your message, or find a wall-mounted station map!",
				...
				prob(35); "[info] It is never a bad idea to visit the medbay if you get injured - small burns and cuts can get infected and become harder to treat! If there's no medical, check the little med stations scattered around the station - with ointments to disinfect cuts and burns, bandages to treat bruises and encourage healing.",
				...
				prob(50); "[info] Our PDAs are useful! You can check weather, SOP, and other important documents in there. Give it a look!",
				prob(40); "[info] The library's computer contains Bingle, an excellent in-game wiki that updates on-compile. Check it for ore processing, food recipes, chemistry, and much more!",
		...
		if("roleplay")
			var/info = "The following is a roleplay-focused tip for playing on EIS Pyraxis! You can disable them using the periodic tips toggle in the Global tab of the character setup! \n"
			return pick(
				...
				prob(25); "[info] While following the SOP is not mandatory, and you are free to break it (albeit, with potential in-character consequences), departments like security and medical do well to be familiar with them! Check your PDA!",
		...
		if("lore")
			var/info = "The following is a tip for understanding the lore of EIS Pyraxis! You can disable them using the periodic tips toggle in the Global tab of the character setup! \n"
			return pick(
				prob(25); "[info] Our lore is somewhat in line with other servers. The year is 2568 (current year+544).",
				prob(25); "[info] Unlike other servers, you don't work for Nanotrasen, instead working for Einstein Engines; a separate company that focuses on power and generator services.",
				prob(10); "[info] Thaler (Taa-ler) is a universal monopoly money. It is backed and supported by Sol Central and its allies. While ubiquitous in frontier worlds, it has an unfavourable exchange rate with most currencies used by well-settled regions, limiting immigration to places such as Earth. https://wiki.chompstation13.net/index.php/Lore"
			)
